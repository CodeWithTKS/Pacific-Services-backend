...
const createSale = async (req, res) => {
    try {
        const { user_id, portalId, services, total_price } = req.body;
        const result = await salesService.createSale({ user_id, portalId, services, total_price });
...
const getSales = async (req, res) => {
    ...
    try {
        const sales = await salesService.getSales(fromDate, toDate, portalId);
...
const updateSale = async (req, res) => {
    try {
        const { user_id, portalId, services, total_price } = req.body;
        const result = await salesService.updateSale(req.params.id, { user_id, portalId, services, total_price });
        if (result) {
